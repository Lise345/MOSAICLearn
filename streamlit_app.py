...
.block-container { max-width: 1180px; padding-top: 1.15rem; padding-bottom: 5rem; }
...
@media (max-width: 620px) { .m-building-grid,.m-evidence-grid,.m-resource-grid,.m-driver-grid{grid-template-columns:1fr}.m-lesson-row{grid-template-columns:38px minmax(0,1fr)}.m-lesson-time{grid-column:2}.m-theory-card{min-height:0}.m-mini-mark{width:30px;height:30px}.m-brandbar{margin-bottom:.8rem} }
</style>
""",
...
def page_home(user):
    ...
    cols = st.columns(3)
    tracks = [
        ("Community", "◫", "Bring people together through a Policy Lab and learn from one another.", "policy-lab"),
        ("Learning", "◎", "Understand the deeper drivers behind current land-use decisions.", "drivers"),
        ("Empowerment", "↝", "Explore future pathways and return with sharper options for action.", "future-pathways"),
    ]
    for col, (title, icon, text, module_id) in zip(cols, tracks):
        with col:
            ...
            if module["status"] == "Available":
                if st.button(f"Explore {title.lower()} →", key=f"home-track-{module_id}", use_container_width=True):
                    navigate("module", module_id)
            else:
                st.button("Coming soon", key=f"home-track-{module_id}", disabled=True, use_container_width=True)
...
def render_theory_story(module_id: str, topic):
    ...
    st.caption("Move the slider or use the arrows. Each step keeps one concept on screen so the theory is easier to scan and revisit.")
    step = st.slider(
        "Theory step",
        min_value=1,
        max_value=len(cards),
...
def page_topic(user):
    ...
    confidence = st.slider(
        "How confident are you that you can use this idea?",
        min_value=1,
        max_value=5,
        value=int(saved.get("confidence") or 3),
        key=f"confidence-{module_id}-{topic['id']}",
    )
...
def page_results(user):
    ...
    st.markdown("<div class='m-section-title'><h2>Learning record</h2><p>A compact view of what you've completed and reflected on.</p></div>", unsafe_allow_html=True)
    for topic in learning_topics(module):
        p = progress.get(topic["id"], {})
        status = "✓ Completed" if p.get("completed") else "Not completed"
        with st.expander(f"{topic['level']} · {topic['title']} · {status}"):
            if p.get("reflection"):
                st.markdown("**Your reflection**")
                st.write(p["reflection"])
            if p.get("confidence"):
                st.caption(f"Confidence: {p['confidence']}/5")
            if not p.get("reflection") and not p.get("confidence"):
                st.caption("No reflection saved for this lesson yet.")
...
def page_community(user):
    ...
    with share_tab:
        ...
        with st.form("community-post-form", border=True):
            module_id = st.selectbox("Related module", available_ids, index=default_index, format_func=lambda mid: MODULES[mid]["short_title"])
            ...
            confirm = st.checkbox("I understand this post is visible to other learners using this prototype.")
            submitted = st.form_submit_button("Publish reflection", type="primary")
        if submitted:
            cleaned = text.strip()
            if len(cleaned) < 20:
                st.error("Write at least a short reflection before publishing.")
            elif not confirm:
                st.error("Please confirm that the post can be shown to other learners.")
            else:
