...
def get_current_user():
    ...
    # Local development fallback. Production should configure OIDC secrets.
    ...
    if not st.session_state.demo_user:
        st.markdown(
            ...
            <div class="m-module-hero learning">
                ...
                <h1>Preview the learner experience</h1>
                <p>OIDC is not configured in this environment, so this local preview uses a temporary demo identity.</p>
            </div>
            """,
            unsafe_allow_html=True,
        )
...
def render_theory_story(module_id: str, topic):
    ...
    embed_summary = bool(topic.get("takeaway") or topic.get("practice_note"))
    ...
    if item.get("summary_slide"):
        ...
        if topic.get("practice_note"):
            practice_feature = (
                ...
                f"<p>{escape(topic['practice_note'])}</p></div>{practice_media}</div>"
            )
        card_html = (
            ...
            "<h3>Key idea</h3>"
            f"<div class='m-key-idea-panel'><p>{escape(topic.get('takeaway', ''))}</p></div>"
